...
from retinanet.dataloader import (Augmenter, Normalizer,Resizer, VOCDetection, collater)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='RetinaNet Evaluation With Pytorch')
    ...
    parser.add_argument('--st', '--score_threshold', default=0.05,
                        type=float, help='Visualization threshold')
    ...
    retinanet = torch.load(resume_path)
    use_gpu = True
    if use_gpu:
    ...
    if(args.dataset == 'Pasadena' or args.dataset == 'mapillary'):
        valid_dataset = VOCDetection(root=args.dataset_root, dataset_name=args.dataset, overfit= args.overfit, image_sets='test',
                                     transform=transforms.Compose([Normalizer(), Resizer()]))
        evaluate(valid_dataset, retinanet, iou_threshold=args.it, score_threshold=args.st)
    else:
        valid_dataset = CocoDataset(root_dir=args.dataset_root, set_name='val2017',
                                    transform=transforms.Compose([Normalizer(), Resizer()]))
        evaluate_coco(valid_dataset, retinanet)